...
if not TMDB_API_KEY:
    raise RuntimeError("TMDB_API_KET missing. Put it in .env as TMDB_API_KEY=aaaa")
...
async def tmdb_get(path:str, params:Dict[str,Any]) -> dict[str,Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"TMDB request error: {type(e).__name__} | {str(e)}",
        )
...
def build_title_to_idx_map(indices: Any) -> Dict[str, int]:
    ...
    try:
        ...
        raise RuntimeError(
            "indices.plk must be dict or pandas Series-like(with .items())"
        )
